...
if (camera_debug_mode) {
	...
	#region BOTTOM SIDE
		...
		var err_size = array_length(error_list);
	...
	#region TOP-LEFT
		...
		switch(Camera.cam_mode) {
			...
			case CMODE.OBJECT_FOLLOWING_H:
				...
				//basic_info[9] = "Vertical Lock: " + string(cam_lock_vert);
			break;
		...
		// Blank space
		var basic_info_len = array_length(basic_info);
		...
		if (shake_current) {
			basic_info_len = array_length(basic_info);
			basic_info[basic_info_len] = "";
	...
	if (cam_mode==CMODE.OBJECT_BORDERS) {
		...
		draw_line_width(cx, cy, cx +rx*g2v/view_total_scale, cy, 3);
		draw_line_width(cx, cy, cx, cy+ry*g2v/view_total_scale, 3);
	}
	#endregion
